class Cell {
  constructor(cell) {
    ...
    for (let i = 1; i <= 3; i++) {
      if (cell.classList.contains('col' + i)) this.column = i;
      if (cell.classList.contains('row' + i)) this.row = i;
    }
    this.id = cell.id;
...
class Line {
  ...
  isWinner() {
    ...
    if (players.length < this.cells.length) return false;
  ...
  winningPlayer() {
    if (this.isWinner()) return this.cells[0].getPlayer();
  }
}

class GameBoard {
  static _root = document.getElementById('board');
  static cells = Array.from(this._root.querySelectorAll('td')).map(
    (cell) => new Cell(cell)
  );
...
GameBoard._root.addEventListener('click', (e) => {
  ...
  if (cell !== null) Game.onCellClick(cell);
});
...
class ScoreBoard {
  ...
  static incrementPlayerWins(player) {
    player.score++;
    document.getElementById('score-' + player.token.toLowerCase()).innerText =
      player.score;
  }
}
...
class Game {
  ...
  static switchPlayer() {
    if (this.currentPlayer === this.playerX) this.currentPlayer = this.playerO;
    else this.currentPlayer = this.playerX;
  ...
  static onCellClick(cell) {
    if (cell.isOpen()) this.selectCell(cell);
  }
  ...
  static selectCell(cell) {
    ...
    if (this.isGameOver()) {
      if (this.isDraw()) this.reportDraw();
      else this.reportWinner(this.winningLine());
    } else {
      this.switchPlayer();
    }
  }
